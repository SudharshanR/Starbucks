/*************
Class: AnniversaryBlendScreen
Created by: Venkataraman
After the customer choose Coffee form, the customer chooses this option to add to myCoffees.
*/
public class AnniversaryBlendScreen implements AppScreen,SubjectInterface{
    // instance variables
    private AppScreen addedAnniversaryBlendScreen;  
...
//    private Bar nameBar = new Bar(0, TIMEBARHEIGHT, THE_WIDTH, NAMEBARHEIGHT, "mycardsbar.png");
    ...
    PImage card = loadImage("AnniversaryBlend.PNG");
	
    private ArrayList<ObserverInterface> observers;
    
    public AnniversaryBlendScreen(){
  ...
  //  private ImageButton balanceBox = new ImageButton(int(THE_WIDTH*.173), int(THE_HEIGHT*.58), int(THE_WIDTH*.654), int(THE_HEIGHT*.1626), card[2]);
   ...
   //     this.drinkMainScreen1 = scr;
    ...
    public void setAddedAnniversaryBlendScreen(AppScreen scr)
    {
        this.addedAnniversaryBlendScreen = scr;
    }
    ...
    // Constructor
    public void AnniversaryBlendScreen()
    {
    ...
    {
      if (x > int(14) && x < int(482) && y > int(593) && y < int(629))
     { 
        notifyObservers();
       appController.setScreen(addedAnniversaryBlendScreen);
      }
